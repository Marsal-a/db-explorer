...
output$ui_navig_filters <- renderUI({
  ...
  wellPanel(
    ...
    returnTextAreaInput("navig_data_filter",
    ...
    fluidRow(
      column(width = 10,actionLink("navig_clearFilters", "Clear filters", icon = icon("sync", verify_fa = FALSE), style = "color:black")),
      column(width = 2,actionLink("navig_help_filter", "", icon = icon("question-circle", verify_fa = FALSE), style = "color:#4b8a8c"))
    ),
    checkboxInput("navig_filterByClick", "Cliquer pour filtrer?", value = F),
    checkboxInput("navig_cumulateFilters", "Accumuler filtres?", value = F)
  )
})
...
output$ui_navig_arrange <- renderUI({
  req(input$navig_table)
  wellPanel(
    returnTextAreaInput("navig_data_arrange",
...
output$ui_navig_view_vars <- renderUI({
  ...
  wellPanel(selectInput(
    "navig_view_vars", "Sélectionner les colonnes :",
    choices = vars,
    selected=vars,
    multiple = TRUE,
...
output$ui_navig_dl_view_tab <- renderUI({
  if(!is.null(NAVIG_displayTable())){
    downloadButton("navig_dl_data",label="Ctrl + S",ic = "download",class = "alignright")
  }else{
    NULL
  }
})
