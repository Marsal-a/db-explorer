...
rstudioapi::jobRunScript(path = "./test_runJob_shiny.R",importEnv = T,exportEnv = "R_GlobalEnv",name = "FIF")
browseURL(url = "http://127.0.0.1:3838")

eval(parse(text = 'invisible(rstudioapi::jobRunScript(path = "./test_runJob_shiny.R",
     importEnv = TRUE, exportEnv = "R_GlobalEnv"));browseURL(url = "http://127.0.0.1:3838");rstudioapi::executeCommand(commandId = "activateConsole")'))
